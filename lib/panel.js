...
const STYLE = `
  ...
  *{box-sizing:border-box}
  ...
  input,select,textarea{background:#0b1120;border:1px solid var(--line);border-radius:6px;color:var(--text);padding:9px 12px;font:13px ui-monospace,Consolas,monospace;flex:1;min-width:0}
...
function quickLinkFields(link = {}) {
    return `
      <div class="field"><label>Kategori</label><input name="category" maxlength="60" required value="${esc(link.category || '')}" placeholder="Uçuş"></div>
      <div class="field"><label>Kategori sırası</label><input name="categoryOrder" type="number" min="0" max="9999" required value="${esc(link.categoryOrder ?? 0)}"></div>
      ...
      <div class="field wide"><label>AI için kısa açıklama (isteğe bağlı)</label><textarea name="description" maxlength="240" placeholder="Bu site hangi işlerde uygundur?">${esc(link.description || '')}</textarea></div>
...
function renderQuickLinks({ account, catalogue, csrf, error = '', notice = '' }) {
    const rows = catalogue.links.length ? catalogue.links.map((link) => `
  <div class="card">
    <div class="row" style="justify-content:space-between">
      <h2>${esc(link.category)} · ${esc(link.name)}</h2>
    ...
    <form method="post" action="/admin/quick-links/save" class="row">
      ...
      ${quickLinkFields(link)}
      <button type="submit">Değişiklikleri kaydet</button>
    </form>
    <form method="post" action="/admin/quick-links/delete" class="row">
      ${csrfField(csrf)}
      <input type="hidden" name="id" value="${esc(link.id)}">
      <button type="submit" class="danger">Kalıcı olarak sil</button>
      <span class="sub">Silmeden yayından kaldırmak için durumu “Gizli” yapın.</span>
    </form>
  </div>`).join('') : '<div class="card"><p class="empty">Katalog boş. Aşağıdaki formdan ilk siteyi ekleyin.</p></div>';
  ...
  <div>
    <h1>Hızlı Linkler</h1>
    <p class="sub">Sürüm ${esc(catalogue.revision)} · Bu global katalog tüm Android uygulamalarına ve AI istemcilerine öneri olarak yayınlanır.</p>
  </div>
  ${banner('err', error)}
  ${banner('ok', notice)}
  <div class="card">
    ...
    <form method="post" action="/admin/quick-links/save" class="row">
      ${csrfField(csrf)}
      ${quickLinkFields({ active: true, categoryOrder: 0, sortOrder: 0 })}
