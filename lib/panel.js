 ...
 *
 * Deliberately absent: another account's browsing audit trail. Operators get
 * aggregate account/tool/status counters, never hosts, request arguments,
 * response content or secrets. `/audit` stays scoped to the signed-in
...
function renderUsageAnalytics({ account, accounts, filters, availableTools, analytics }) {
  ...
  <div class="card"><div class="card-head"><div><h2>Kullanıcı ve Araç Günlüğü</h2><p class="sub">Hesap + AI oturumu + araç düzeyinde toplulaştırılmış kayıtlar; en yeni kullanım üstte.</p></div><span class="pill">en fazla 500 satır</span></div><div class="table-wrap"><table><thead><tr><th>Kullanıcı</th><th>AI oturumu</th><th>Araç</th><th>Toplam</th><th>Başarılı</th><th>Hata / kota</th><th>Ort. süre</th><th>Son kullanım</th></tr></thead><tbody>${activityRows}</tbody></table></div>
