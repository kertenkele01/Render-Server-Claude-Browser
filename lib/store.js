    ...
    return {
        ...
        async setDeviceAccount(id, accountId) {
            ...
            for (const binding of bindings) {
                ...
                const others = deviceIdsForClient(c.id).filter((deviceId) => deviceId !== id);
                if (c.deviceId === id && others.length === 0) {
                    // Preserve the old one-device logout behaviour: locally
                    // minted clients keep routing as unclaimed upgrade data.
                    c.accountId = null;
                } else {
                    data.clientDevices.delete(bindingKey(c.id, id));
                    if (c.deviceId === id) {
                        const next = others[0];
                        c.deviceId = next;
                        c.accountId = data.devices.get(next)?.accountId || c.accountId;
                    }
                }
            }
            save();
        ...
        async replaceDeviceClients(deviceId, list) {
            ...
            list.forEach((c) => {
                ...
                if (existing) {
                    ...
                    if (!isOrigin && !sameClaimedAccount(existing, device)) {
                        data.clientDevices.delete(bindingKey(c.id, deviceId));
                        conflicts.push(c.id);
                        return;
                    }
                    if (existing.secretHash !== c.secretHash) {
                        // Only the phone that minted the credential may rotate
                        // its hash. Another account device may share the same
                        // credential, never replace it.
                        if (!isOrigin) {
                            data.clientDevices.delete(bindingKey(c.id, deviceId));
                            conflicts.push(c.id);
                            return;
        ...
        async upsertClient(client) {
            ...
            if (existing) {
                ...
                if (!isOrigin && !sameClaimedAccount(existing, device)) return null;
...
async function createPostgresDriver(connectionString) {
    ...
    const rowClient = (r) => {
        ...
        if (r.device_id && !deviceIds.includes(r.device_id)) deviceIds.unshift(r.device_id);
        return {
        ...
        async setDeviceAccount(id, accountId) {
            ...
            try {
                ...
                const { rows } = await db.query(
                    'UPDATE devices SET account_id = $2 WHERE id = $1 RETURNING *',
                    [id, accountId]
                );
                if (!rows[0]) {
                    ...
                    for (const c of bound.rows) {
                        const alternatives = await db.query(
                            `SELECT cd.device_id, d.account_id
                             ...
                             ORDER BY cd.created_at, cd.device_id
                             LIMIT 1`,
                            [c.id, id]
                        );
                        const next = alternatives.rows[0];
                        if (c.device_id === id && !next) {
                            await db.query('UPDATE clients SET account_id = NULL WHERE id = $1', [c.id]);
                        } else {
                            await db.query(
                            ...
                            if (c.device_id === id && next) {
                                await db.query(
                                    ...
                                    [c.id, next.device_id, next.account_id]
                                );
                            }
                        }
                    }
            ...
            try {
                ...
                for (const c of list) {
                    ...
                    if (existing) {
                        ...
                        if ((!isOrigin && !sameAccount) || (!isOrigin && existing.secret_hash !== c.secretHash)) {
                            await db.query(
                                'DELETE FROM client_devices WHERE client_id = $1 AND device_id = $2',
                                [c.id, deviceId]
                            );
                ...
                await db.query(
                    'DELETE FROM clients c WHERE NOT EXISTS (SELECT 1 FROM client_devices cd WHERE cd.client_id = c.id)'
                );
                await db.query('COMMIT');
                return { accepted, conflicts };
            } catch (e) {
