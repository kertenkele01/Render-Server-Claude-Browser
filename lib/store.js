...
async function createPostgresDriver(connectionString) {
    ...
    const configuredCa = String(process.env.DATABASE_CA_CERT || '').replace(/\\n/g, '\n').trim();
    const pool = new Pool({
        connectionString,
        // Remote database traffic must authenticate its peer. DATABASE_CA_CERT
        // accepts a provider CA (including a PEM stored with escaped newlines)
        // when the platform certificate is not in Node's normal trust store.
