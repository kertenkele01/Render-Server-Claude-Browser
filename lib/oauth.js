...
<body>
  <main class="card">
    <h1>${canSubmit ? 'Bağlantıyı onayla' : 'Bağlantı kurulamadı'}</h1>
    <p class="sub">${canSubmit
        ? `<span class="who">${escapeHtml(clientName || 'Bir MCP istemcisi')}</span> telefonunuzdaki tarayıcıya bağlanmak istiyor.`
        : 'OAuth bağlantı isteği doğrulanamadı.'}</p>
    ...
    <p class="foot">${canSubmit ? `Bu sayfa parola istemez ve hiçbir yetki vermez. Erişim anahtarını
    ...
    kaldırabilirsiniz.` : 'Bağlantıyı başlatan uygulamaya geri dönüp yeniden deneyin.'}</p>
  </main>
  <script>
 ...
 */
function renderOAuthRedirectPage(callbackUrl, clientName = 'bağlantıya') {
...
<title>${safeClientName} dönülüyor</title>
<style>
...
<body>
  <main>
    <h1>Kod kabul edildi</h1>
    <p>${safeClientName} dönülüyor. Otomatik geçiş olmazsa aşağıdaki düğmeye dokunun.</p>
    <a id="continue" href="${htmlTarget}" rel="noreferrer">${safeClientName} dön</a>
  </main>
  <script>
    (function () {
