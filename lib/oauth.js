...
function validateClientMetadataDocument(clientId, body) {
    ...
    if (String(body.token_endpoint_auth_method || 'none') !== 'none') {
        return { error: 'Bu sunucu CIMD istemcileri için yalnızca token_endpoint_auth_method=none destekliyor.' };
    }
    return { record: { id: clientId, name: String(body.client_name).substring(0, 80), redirectUris: redirectUris.map(String), cimd: true } };
}
