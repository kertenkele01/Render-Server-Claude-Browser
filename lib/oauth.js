...
function redirectUriProblem(value) {
    ...
    if (url.username || url.password) return 'redirect_uri kullanıcı bilgisi içeremez.';
 ...
 * part of the comparison. Scheme, host, path and query must still match;
 * fragments and embedded credentials are refused.
 */
function redirectMatches(registered, requested) {
    if (redirectUriProblem(registered) || redirectUriProblem(requested)) return false;
    ...
    return String(registered) === String(requested);
}
...
function validateClientMetadataDocument(clientId, body) {
    if (!body || typeof body !== 'object' || Array.isArray(body)) {
        return { error: 'İstemci metadata belgesi geçerli bir JSON nesnesi değil.' };
    }
    if (String(body.client_id || '') !== clientId) {
        return { error: 'Metadata belgesindeki client_id belge adresiyle aynı değil.' };
    }
    if (!String(body.client_name || '').trim()) return { error: 'Metadata belgesinde client_name eksik.' };
    const redirectUris = Array.isArray(body.redirect_uris) ? body.redirect_uris : [];
    if (!redirectUris.length || redirectUris.length > 10) {
        return { error: 'Metadata belgesinde 1-10 redirect_uri bulunmalı.' };
    }
    for (const uri of redirectUris) {
        const problem = redirectUriProblem(uri);
    ...
    // SEP-3149's transition format lets a CIMD client publish every method it
    // can use in the plural field while keeping the old singular field as a
    // preference. ChatGPT currently publishes ["none", "private_key_jwt"]
    // there and prefers private_key_jwt. This authorization server advertises
    // only "none", so the intersection is still valid and the public-client
    // PKCE flow must be selected rather than rejecting the legacy preference.
    let authMethods;
    if (Object.prototype.hasOwnProperty.call(body, 'token_endpoint_auth_methods_supported')) {
        if (!Array.isArray(body.token_endpoint_auth_methods_supported) ||
            body.token_endpoint_auth_methods_supported.length === 0 ||
            body.token_endpoint_auth_methods_supported.length > 10 ||
            body.token_endpoint_auth_methods_supported.some((method) => typeof method !== 'string' || !method.trim())) {
            return { error: 'Metadata belgesindeki token_endpoint_auth_methods_supported geçerli bir yöntem listesi olmalı.' };
        }
        authMethods = body.token_endpoint_auth_methods_supported.map(String);
    } else {
        // Older CIMD documents expose only the singular RFC 7591 field. Its
        // historical default is "none", which preserves existing clients.
        authMethods = [String(body.token_endpoint_auth_method || 'none')];
    }
    if (!authMethods.includes('none')) {
        return { error: 'Bu istemci, sunucunun desteklediği token endpoint doğrulama yöntemi olan "none" seçeneğini sunmuyor.' };
    }
    return {
        record: {
            id: clientId,
            name: String(body.client_name).substring(0, 80),
            redirectUris: redirectUris.map(String),
...
/** Fetch an HTTPS CIMD while pinning DNS to a checked public address. */
async function resolveClientMetadata(clientId) {
    let url;
    try { url = new URL(clientId); } catch (e) { return { error: 'client_id geçerli bir URL değil.' }; }
    if (url.protocol !== 'https:' || !url.pathname || url.pathname === '/' || url.username || url.password || url.hash) {
        return { error: 'CIMD client_id, path içeren temiz bir HTTPS adresi olmalı.' };
    }
    let addresses;
    try { addresses = await dns.lookup(url.hostname, { all: true }); }
    catch (e) { return { error: 'İstemci metadata adresinin DNS kaydı çözülemedi.' }; }
    if (!addresses.length || addresses.some((item) => isPrivateAddress(item.address))) {
        return { error: 'İstemci metadata adresi genel bir internet adresine yönelmeli.' };
    }
    const chosen = addresses[0];
    return new Promise((resolve) => {
        const req = https.request(url, {
            method: 'GET',
            headers: { accept: 'application/json', 'user-agent': 'mcp-android-bridge/1.0' },
            lookup(_hostname, options, callback) {
                // Recent Node releases ask custom lookup functions for an
                // array (`all: true`) even though the socket will use one
                // address. Preserve that contract while still pinning the
                // connection to the public address checked above.
            ...
            if (res.statusCode !== 200) {
                res.resume();
                return resolve({ error: `İstemci metadata adresi HTTP ${res.statusCode} döndürdü.` });
            }
            let raw = '';
            res.setEncoding('utf8');
            res.on('data', (chunk) => {
                raw += chunk;
                if (raw.length > 64 * 1024) req.destroy(new Error('metadata_too_large'));
            });
            res.on('end', () => {
                try { resolve(validateClientMetadataDocument(clientId, JSON.parse(raw))); }
                catch (e) { resolve({ error: 'İstemci metadata belgesi geçerli JSON değil.' }); }
            });
        });
        req.setTimeout(5000, () => req.destroy(new Error('metadata_timeout')));
        req.on('error', () => resolve({ error: 'İstemci metadata belgesi alınamadı.' }));
        req.end();
    });
}
...
/** Pin the public origin, or use forwarding headers only from trusted peers. */
function originOf(req) {
    const configured = require('./network').publicOrigin();
    if (configured) return configured;
    const trusted = require('./limits').trustProxy(req.socket?.remoteAddress || '');
    const forwardedProto = trusted ? String(req.headers['x-forwarded-proto'] || '').split(',')[0].trim() : '';
    const proto = ['https', 'http'].includes(forwardedProto) ? forwardedProto : (req.secure ? 'https' : 'http');
    const host = String((trusted && req.headers['x-forwarded-host']) || req.headers.host || '').split(',')[0].trim();
    return `${proto}://${host}`;
}
 ...
 */
function protectedResourceMetadata(origin, suffix = '') {
    return {
...
// OAuth is the only user-facing relay page. Keep its copy in a small catalog
// rather than mixing language decisions into authorization logic. Adding a
// language means adding one catalog entry and exposing its code here.
const OAUTH_LANGUAGES = Object.freeze(['tr', 'en']);

function normaliseLanguage(value, fallback = 'tr') {
    const candidate = String(value || '').trim().toLowerCase().split(/[-_]/)[0];
    return OAUTH_LANGUAGES.includes(candidate) ? candidate :
        (OAUTH_LANGUAGES.includes(fallback) ? fallback : 'tr');
}

function languageFromAcceptLanguage(value) {
    const requested = String(value || '').split(',').map((part) => part.trim().split(';')[0]);
    for (const item of requested) {
        const language = String(item || '').toLowerCase().split(/[-_]/)[0];
        if (OAUTH_LANGUAGES.includes(language)) return language;
    }
    return 'tr';
}

const OAUTH_COPY = Object.freeze({
    tr: Object.freeze({
        approveTitle: 'Bağlantıyı onayla', failedTitle: 'Bağlantı kurulamadı',
        unknownClient: 'Bir MCP istemcisi', wantsAccess: 'telefonunuzdaki tarayıcıya bağlanmak istiyor.',
        invalidRequest: 'OAuth bağlantı isteği doğrulanamadı.', codeLabel: 'Telefondaki bağlantı kodu',
        codeHint: '8 karakterlik kod (ör. D2J5-QT5R)', connect: 'Bağlan',
        steps: [
            'Telefonda uygulamayı açın.',
            '<strong>Ayarlar → MCP → Oturumlar</strong>\'a gidin.',
            'Kullanmak istediğiniz istemci kartında <strong>“OAuth bağlantı kodu”</strong>na dokunun.',
            'Ekranda çıkan kodu buraya yazın. Kod 5 dakika geçerlidir ve bir kez kullanılır.'
        ],
        safety: 'Bu sayfa parola istemez ve hiçbir yetki vermez. Erişim anahtarını telefonunuz üretir; bu adım yalnızca onu istemciye teslim eder. İzinleri istediğiniz zaman aynı istemci kartından değiştirebilir veya erişimi kaldırabilirsiniz.',
        retry: 'Bağlantıyı başlatan uygulamaya geri dönüp yeniden deneyin.',
        language: 'Dil', accepted: 'Kod kabul edildi', returning: 'dönülüyor.',
        transition: 'Otomatik geçiş olmazsa aşağıdaki düğmeye dokunun.', returnButton: 'dön'
    }),
    en: Object.freeze({
        approveTitle: 'Approve connection', failedTitle: 'Connection could not be established',
        unknownClient: 'An MCP client', wantsAccess: 'wants to connect to the browser on your phone.',
        invalidRequest: 'The OAuth connection request could not be verified.', codeLabel: 'Connection code from your phone',
        codeHint: '8-character code (for example D2J5-QT5R)', connect: 'Connect',
        steps: [
            'Open the app on your phone.',
            'Go to <strong>Settings → MCP → Sessions</strong>.',
            'Tap <strong>“OAuth connection code”</strong> on the client card you want to use.',
            'Enter the displayed code here. It is valid for five minutes and can be used once.'
        ],
        safety: 'This page never asks for your password and grants no permissions. Your phone creates the access key; this step only delivers it to the client. You can change permissions or revoke access from the same client card at any time.',
        retry: 'Return to the app that started the connection and try again.',
        language: 'Language', accepted: 'Code accepted', returning: 'Returning to',
        transition: 'If the redirect does not happen automatically, use the button below.', returnButton: 'Return to'
    })
});

const OAUTH_ERROR_ENGLISH = Object.freeze({
    'client_id eksik.': 'client_id is missing.',
    'İstemci kaydı okunamadı.': 'The client registration could not be read.',
    'Bu client_id kayıtlı değil. İstemcinin yeniden kayıt olması gerekiyor.': 'This client_id is not registered. The client must register again.',
    'redirect_uri eksik.': 'redirect_uri is missing.',
    'redirect_uri bu istemci için kayıtlı adreslerle eşleşmiyor.': 'redirect_uri does not match an address registered for this client.',
    'Yalnızca response_type=code destekleniyor.': 'Only response_type=code is supported.',
    'code_challenge zorunlu: bu sunucu PKCE olmadan yetkilendirme yapmaz.': 'code_challenge is required: this server does not authorize without PKCE.',
    'code_challenge_method yalnızca S256 olabilir.': 'code_challenge_method must be S256.',
    'Kod 8 karakter olmalı. Telefondaki kodu olduğu gibi yazın; büyük/küçük harf ve tire fark etmez.': 'The code must contain 8 characters. Enter it as shown on the phone; letter case and the dash do not matter.',
    'Kod geçersiz ya da süresi dolmuş. Telefondan yeni bir kod alın — kodlar 5 dakika geçerlidir ve bir kez kullanılır.': 'The code is invalid or expired. Request a new code from the phone; codes are valid for five minutes and one use.',
    'Bu kodun bağlı olduğu erişim anahtarı artık geçerli değil. Telefondan yeni bir kod alın.': 'The access key associated with this code is no longer valid. Request a new code from the phone.'
});

function oauthCopy(language) {
    return OAUTH_COPY[normaliseLanguage(language)] || OAUTH_COPY.tr;
}

function localizeOAuthMessage(message, language) {
    const raw = String(message || '');
    if (normaliseLanguage(language) !== 'en') return raw;
    const limited = raw.match(/^Çok fazla hatalı kod denendi\. (\d+) saniye sonra tekrar deneyin\.$/);
    if (limited) return `Too many incorrect codes were entered. Try again in ${limited[1]} seconds.`;
    return OAUTH_ERROR_ENGLISH[raw] || raw;
}
 ...
 */
function renderAuthorizePage({ clientName, hidden, error, notice, language = 'tr' }) {
    const lang = normaliseLanguage(language);
    const copy = oauthCopy(lang);
    const context = hidden || {};
    const canSubmit = Boolean(
        context.client_id && context.redirect_uri &&
        context.code_challenge && context.response_type === 'code'
    );
    ...
    const languageHref = (target) => {
        const query = new URLSearchParams({ ...context, lang: target }).toString();
...
<title>${canSubmit ? copy.approveTitle : copy.failedTitle}</title>
<style>
  :root {
  ...
  * { box-sizing: border-box; }
  ...
  .languages { display: flex; justify-content: flex-end; gap: 7px; margin: -8px 0 14px; font-size: 12px; }
  .languages a { color: var(--muted); text-decoration: none; padding: 3px 7px; border: 1px solid var(--line); border-radius: 99px; }
  .languages a[aria-current=true] { color: var(--ink); font-weight: 700; border-color: var(--accent); }
</style>
</head>
<body>
  <main class="card">
    <nav class="languages" aria-label="${copy.language}">
      <a href="${escapeHtml(languageHref('tr'))}" lang="tr" aria-current="${lang === 'tr'}">TR</a>
      <a href="${escapeHtml(languageHref('en'))}" lang="en" aria-current="${lang === 'en'}">EN</a>
    </nav>
    <h1>${canSubmit ? copy.approveTitle : copy.failedTitle}</h1>
    <p class="sub">${canSubmit
        ? `<span class="who">${escapeHtml(clientName || copy.unknownClient)}</span> ${copy.wantsAccess}`
        : copy.invalidRequest}</p>

    ${error ? `<div class="msg bad">${escapeHtml(localizeOAuthMessage(error, lang))}</div>` : ''}
    ${notice ? `<div class="msg good">${escapeHtml(notice)}</div>` : ''}

    ${canSubmit ? `<form method="post" action="${escapeHtml(formAction)}" autocomplete="off">
            ${fields}
      ...
      <input id="code" name="code" type="text" inputmode="text" autocapitalize="characters"
             ...
             title="${copy.codeHint}" required autofocus>
      <button type="submit">${copy.connect}</button>
    </form>` : ''}
    ...
    <p class="foot">${canSubmit ? copy.safety : copy.retry}</p>
  </main>
  <script>
...
/**
 * Claude's hosted connector has intermittently kept the authorization form's
 * POST method while following a callback redirect. Its callback accepts GET,
 * so a successful authorization can otherwise look like a dead button even
 * though the phone code has already been consumed.
 *
 * This small transition document initiates a fresh top-level navigation in
 * several browser-compatible ways and leaves a normal link as a fallback. It
 * contains only the short-lived authorization callback URL; the bearer secret
 * is still released solely by the token endpoint after PKCE verification.
 */
function renderOAuthRedirectPage(callbackUrl, clientName = 'bağlantıya', language = 'tr') {
    const lang = normaliseLanguage(language);
    const copy = oauthCopy(lang);
    const target = String(callbackUrl || '');
    const htmlTarget = escapeHtml(target);
    const safeClientName = escapeHtml(clientName || 'bağlantıya');
    // JSON string syntax is safe inside this script after neutralising the two
    // characters that could terminate a script element in HTML parsing.
...
<title>${lang === 'en' ? `${copy.returning} ${safeClientName}` : `${safeClientName} ${copy.returning}`}</title>
<style>
  :root { color-scheme: light dark; --bg: #f5f6f8; --card: #fff; --ink: #16181d; --muted: #5c6270; --accent: #2f6df6; }
  @media (prefers-color-scheme: dark) { :root { --bg: #101216; --card: #191c22; --ink: #eceef2; --muted: #a6adba; --accent: #79a0ff; } }
  * { box-sizing: border-box; }
  body { margin: 0; min-height: 100vh; display: grid; place-items: center; padding: 24px; background: var(--bg); color: var(--ink); font: 15px/1.6 system-ui, -apple-system, "Segoe UI", sans-serif; }
  main { width: 100%; max-width: 440px; padding: 28px; text-align: center; background: var(--card); border-radius: 14px; }
  h1 { margin: 0 0 8px; font-size: 20px; }
  p { margin: 0 0 20px; color: var(--muted); }
  a { display: inline-block; padding: 12px 18px; border-radius: 10px; background: var(--accent); color: #fff; font-weight: 650; text-decoration: none; }
</style>
</head>
<body>
  <main>
    <h1>${copy.accepted}</h1>
    <p>${lang === 'en' ? `${copy.returning} ${safeClientName}. ${copy.transition}` : `${safeClientName} ${copy.returning} ${copy.transition}`}</p>
    <a id="continue" href="${htmlTarget}" rel="noreferrer">${lang === 'en' ? `${copy.returnButton} ${safeClientName}` : `${safeClientName} ${copy.returnButton}`}</a>
  </main>
  <script>
    (function () {
      ...
      setTimeout(function () { try { window.location.assign(target); } catch (_) {} }, 500);
    })();
  </script>
</body>
</html>`;
